...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="w-72 glassmorphic text-white flex flex-col p-4 h-[calc(100vh-2rem)] m-4 rounded-2xl shadow-2xl">
      ...
      <div className="flex-grow space-y-2 overflow-y-auto pr-1">
        ...
        <div className={`collapsible-content ${historyOpen && history.length > 0 ? 'open' : ''}`}>
            <div className="space-y-1">
                ...
                <button key={h.id} onClick={() => onLoadChat(h.id)} className="w-full text-left text-sm p-3 rounded-md text-gray-400 bg-black/20 hover:bg-gray-700/70 transition truncate">
                    {h.summary}
                </button>
                ))}
            </div>
